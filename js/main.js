...
ScrollTrigger.create({
  ...
  onUpdate: self => {
    // fly in from left
    const vw = window.innerWidth/ 100;
    $('#developer-link').style.transform = `translateX(${(vw * 40 * (1 - self.progress))}px)`;
...
ScrollTrigger.create({
  ...
  onUpdate: self => {
    // fly in from left
    const vw = window.innerWidth/ 100;
    $('#journalist-link').style.transform = `translateX(${0.01 + (vw * 40 * (1 - self.progress))}px)`;
    // set opacity from 0 to 1
...
ScrollTrigger.create({
  ...
  onUpdate: self => {
    // fly in from left
    const vw = window.innerWidth/ 100;
    $('#cyclist-link').style.transform = `translateX(${0.01 + (vw * 40 * (1 - self.progress))}px)`;
    // set opacity from 0 to 1
...
ScrollTrigger.create({
  ...
  onUpdate: self => {
    ...
    // scale from center
    $('#big-text').style.transform = `scale(${1 + 140 * (self.progress * self.progress)})`;
  },
  onEnter: self => {
...
$('#fun').addEventListener('click', () => {
  ...
  if (funCount == 5) {
    ...
    setTimeout(() => {
      const x = ($('#super-confetti-button').getBoundingClientRect().x + $('#super-confetti-button').getBoundingClientRect().width / 2)/window.innerWidth;
      const y = ($('#super-confetti-button').getBoundingClientRect().y + $('#super-confetti-button').getBoundingClientRect().height / 2)/window.innerHeight;
      confetti({
        particleCount: 100,
        startVelocity: 30,
        spread: 360,
...
addEventListener('mousemove', (e) => {
  ...
  const xCenter = ($('#contact-button').getBoundingClientRect().x + $('#contact-button').getBoundingClientRect().width / 2)/window.innerWidth;
  const yCenter = ($('#contact-button').getBoundingClientRect().y + $('#contact-button').getBoundingClientRect().height / 2)/window.innerHeight;

  // set dist to x-xCenter unless it's > 1, then set it to 1
  const xDist = Math.abs(x-xCenter) > 1 ? Math.sign(x-xCenter) : x-xCenter;
  const yDist = Math.abs(y-yCenter) > 1 ? Math.sign(y-yCenter) : y-yCenter;
  
  // translate the button towards the mouse, but don't go too far using percent = =-0.9^{(y-yCenter)}+1
  $('#contact-button').style.transform = `translate(${(xDist) * 50}%, ${(yDist) * 50}%)`;
});